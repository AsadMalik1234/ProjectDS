        ...
        try {
            ...
            new Thread() {
                public void run() {
                    ...
                    Iterator<BedStatus> response = blockingStub.warm(request); //make request 
                    while (response.hasNext()) { // while loop is use to get multipple response  
                        ui.append(response.next().toString());
                    }
                }
            }.start(); // use new thread to it will  not affect any other thing running on computer 
                        // 

            Empty request = Empty.newBuilder().build(); // blocking requrst one thing in one thing out 
